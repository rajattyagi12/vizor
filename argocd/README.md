...
Each root chart renders **standalone Applications** (same level as the ApplicationSet) plus one **ApplicationSet** that generates the remaining child apps. Standalone app **names are env-prefixed** (e.g. `vizor-dev-redis`, `vizor-uat-secrets`) so multiple envs can coexist in the same Argo CD instance.

1. **vizor-{env}-redis** (`-3`) — Standalone. Redis (CloudPirates OSS, OCI); service `vizor-redis-master` for Dapr/SignalR.
2. **vizor-{env}-secrets** (`-2`) — **Standalone** (not in the ApplicationSet). Creates the Secret `vizor-secrets` and Dapr SFTP component; chart path `helm/vizor-secrets`. Automated sync is disabled (no prune/selfHeal). Uses [ignoreDifferences](https://argo-cd.readthedocs.io/en/stable/operator-manual/cluster-bootstrapping/#ignoring-differences-in-child-applications) for Secret `/data` and `RespectIgnoreDifferences=true`, so in-cluster or external changes to Secret data do not trigger out-of-sync. **Why at root:** If vizor-secrets were generated by the ApplicationSet, removing it from the generator list (or a template mistake) would cause the ApplicationSet controller to delete the Application; the finalizer would then delete the Secret and all credentials. Keeping it as a root-level Application (like Redis) avoids that risk.
3. **vizor-{env}-mailhog** (`-2`) — **Optional.** Standalone Mailhog (SMTP capture + Web UI). Set `mailhog.enabled: false` in root values to omit (e.g. UAT/prod); default when unset is to include.
4. **vizor-{env}-sftpgo** (wave `1`) — Standalone SFTPGo when used for that env. There is no root-level `sftpgo.enabled` toggle; the Application is always rendered when the template is present. Per-env omission could be added later if needed.
5. **ApplicationSet** generates: `vizor-foundation` (`-2`)
6. `vizor-data-init` (`-1`), 7. `vizor-identity` (`0`), 8. `vizor-platform-support` (`1`), 9. `vizor-apps` (`2`), 10. `vizor-traffic-autoscale` (`3`)
